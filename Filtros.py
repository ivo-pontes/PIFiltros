		...
		for i in range(0,tamM):
			for j in range(0,tamN):
				if i != 0 and j != 0 and i != (tamM-1) and j != (tamN-1):
					...
					saida[i][j] = self.matriz[i-1][j-1]*mascara[0][0] + self.matriz[i-1][j]*mascara[0][1] + self.matriz[i-1][j+1]*mascara[0][2]
					saida[i][j] += self.matriz[i][j-1]*mascara[1][0] + self.matriz[i][j]*mascara[1][1] + self.matriz[i][j+1]*mascara[1][2]
					saida[i][j] += self.matriz[i+1][j-1]*mascara[2][0] + self.matriz[i+1][j]*mascara[2][1] + self.matriz[i+1][j+1]*mascara[2][2]
					saida[i][j] = int(saida[i][j]/9)
					if saida[i][j] < 0:
						saida[i][j] = 0
